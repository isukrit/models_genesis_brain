...
for i, fold in enumerate(tqdm(conf.train_fold)):
    file_name = "batch_CT_"+str(conf.scale)+"_"+str(conf.input_rows)+"x"+str(conf.input_cols)+"x"+str(conf.input_deps)+"_"+str(fold)+".npy"
...
for epoch in range(intial_epoch,conf.nb_epoch):
	...
	if valid_loss < best_loss:
		...
		},os.path.join(conf.model_path, "Genesis_Head_CT.pt"))
		print("Saving model ",os.path.join(conf.model_path,"Genesis_Head_CT.pt"))
	else:
