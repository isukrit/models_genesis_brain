...
from utils import *
import unet3d
from config_MRI import models_genesis_config
...
for i, fold in enumerate(tqdm(conf.train_fold)):
    x_file_name =      "batch_"                   + str(conf.scale) + "_" + str(conf.input_rows) + "x" + str(conf.input_cols) + "x" + str(conf.input_deps) + "_" + str(fold) + ".npy"
    labels_file_name = "batch_segmentation_true_" + str(conf.scale) + "_" + str(conf.input_rows) + "x" + str(conf.input_cols) + "x" + str(conf.input_deps) + "_" + str(fold) + ".npy"
...
base_model = nn.DataParallel(base_model, device_ids = [i for i in range(torch.cuda.device_count())])
criterion = mIoULoss() #nn.CrossEntropyLoss()
optimizer = torch.optim.SGD(base_model.parameters(), conf.lr, momentum=0.9, weight_decay=0.0, nesterov=False)
...
for epoch in range(intial_epoch, conf.nb_epoch):
    ...
    for iteration, bi in enumerate(range(0, y_.shape[0] - conf.batch_size, conf.batch_size)):
        ...
        if (iteration + 1) % conf.batch_size ==0:
    ...
    if test_loss < best_loss:
        ...
        }, os.path.join(conf.model_path, "Genesis_Head_MRI_segmentation.pt"))
        print("Saving model ", os.path.join(conf.model_path,"Genesis_Head_MRI_segmentation.pt"))
    ...
    with open(os.path.join(conf.model_path, 'Genesis_Head_MRI_segmentation_logs.csv'), 'a') as out_stream:
        out_stream.write(str(epoch) + ', ' + str(train_loss) + ', ' + str(test_loss) + '\n')
